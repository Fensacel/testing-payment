...
@section('page-description', 'Update promo code information')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.promo-codes.update', $promoCode) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="number" name="discount_percentage" value="{{ old('discount_percentage', $promoCode->discount_percentage) }}" required min="1" max="100"
            ...
            <div>
                ...
                <input type="number" name="max_uses" value="{{ old('max_uses', $promoCode->max_uses) }}" required min="1"
                ...
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-sm text-gray-500 mt-1">Current usage: {{ $promoCode->used_count }}</p>
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Expiration Date *</label>
                <input type="date" name="expires_at" value="{{ old('expires_at', \Carbon\Carbon::parse($promoCode->expires_at)->format('Y-m-d')) }}" required
